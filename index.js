...
$(document).ready(function () {
  $("#fullpage").fullpage({
    sectionsColor: ["#1bbc9b", "#4BBFC3", "#1bbc9b", "#0fff"],
  });
});
...
topics[currentTopic].style.opacity = 1;
...
for (var i = 0; i < topics.length; i++) {
  splitLettersTopic(topics[i]);
}

function changeTopic() {
  var ct = topicArray[currentTopic];
  var nt =
    currentTopic == topics.length - 1
      ? topicArray[0]
      : topicArray[currentTopic + 1];
  for (var i = 0; i < ct.length; i++) {
    animateLetterOut("letterTopic", ct, i);
  }

  for (var i = 0; i < nt.length; i++) {
    nt[i].className = "letter behind";
    nt[0].parentElement.style.opacity = 1;
    animateLetterIn("letterTopic", nt, i);
  }

  currentTopic = currentTopic == topicArray.length - 1 ? 0 : currentTopic + 1;
}
...
function changeWord() {
  ...
  for (var i = 0; i < cw.length; i++) {
    animateLetterOut("letter", cw, i);
  }

  for (var i = 0; i < nw.length; i++) {
    nw[i].className = "letter behind";
    nw[0].parentElement.style.opacity = 1;
    animateLetterIn("letter", nw, i);
  }

  currentWord = currentWord == wordArray.length - 1 ? 0 : currentWord + 1;
}

function animateLetterOut(letter, cw, i) {
  setTimeout(function () {
    cw[i].className = letter + " out";
  }, i * 80);
}

function animateLetterIn(letter, nw, i) {
  setTimeout(function () {
...
function splitLettersTopic(topic) {
  var content = topic.innerHTML;
  topic.innerHTML = "";
  var letters = [];
  for (var i = 0; i < content.length; i++) {
    var letter = document.createElement("span");
    letter.className = "letterTopic";
    letter.innerHTML = content.charAt(i);
    topic.appendChild(letter);
    letters.push(letter);
  }

  topicArray.push(letters);
}
